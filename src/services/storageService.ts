import type { AppSettings, Folder, QueueItem, StorageKey } from "@/types";
import { GeminiModel, STORAGE_KEYS, SidebarPosition, ThemeMode } from "@/types";
    ...
    templates: [
      {
        id: "t1",
        name: "8K Realistic",
        text: "hyper-realistic, 8k resolution, cinematic lighting",
        createdAt: Date.now(),
    ...
    templates: [
      {
        id: "t2",
        name: "Studio Ghibli",
        text: "hand-drawn anime style, ghibli aesthetics, soft colors",
        createdAt: Date.now(),
        lastEditedAt: Date.now(),
        timesUsed: 0,
        images: [],
...
export async function updateFolder(id: string, updates: Partial<Folder>): Promise<boolean> {
  const folders = await getFolders();
  const updatedFolders = folders.map((folder) => (folder.id === id ? { ...folder, ...updates } : folder));
